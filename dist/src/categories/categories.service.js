...
let CategoriesService = exports.CategoriesService = class CategoriesService {
    ...
    async create(createCategoryInput) {
        const uuid = createCategoryInput.id ? createCategoryInput.id : (0, uuid_1.v4)();
        const existingCategory = categories_1.categories.find((category) => category.id === uuid);
        if (existingCategory) {
    ...
    async updateOneById(id, updateCategoryInput) {
        const category = await this.findOneById(id);
        if (!category) {
            throw new common_1.NotFoundException('Category not found');
        }
